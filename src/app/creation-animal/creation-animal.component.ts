...
@Component({
  ...
  standalone: true,
  imports: [ReactiveFormsModule, CommonModule, HttpClientModule, FormsModule, NgSelectModule, MatFormFieldModule, MatSelectModule,],
...
export class CreationAnimalComponent implements OnInit {
  ...
  constructor(private http: HttpClient, private raceService: RaceService, private especeService: EspeceService, private natureService: NatureService) {
...
// récupérer la liste des especes
  afficherEspece() {
    this.especeService.getAll().then(data => {
      this.especes = data
    }).catch();
  }
...
// récupérer la liste des races
  afficherRaces() {
    this.raceService.getAll().then(data => {
      this.races = data
    }).catch();
  }
...
// récupérer la liste des natures
  ...
  onSubmit() {
    ...
    if(raceId)
      postForm.race = this.getRaceById(raceId)
    console.log(postForm);
    
    if (this.creationAnimal.valid) {
      this.http.post('http://localhost:8080/api/compagnons/save', postForm)
        .subscribe({
          next: (res) => {
